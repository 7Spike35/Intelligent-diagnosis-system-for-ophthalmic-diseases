...
df = pd.read_excel("F:\Retinal-disease-foundational-model-for-ODIR2019-main\data\ODIR-5K_Training_Annotations(Updated)_V2.xlsx")
...
def crop_train_images(df, train_dir, cropped_dir):
    ...
    for file_name in os.listdir(train_dir):
        if file_name.lower().endswith((".jpg", ".png", ".jpeg")):
            input_path = os.path.join(train_dir, file_name)
            output_path = os.path.join(cropped_dir, file_name)
            left_img_check = fov_extraction(input_path, file_name, cropped_dir)
            #right_img_check = fov_extraction(input_path, row['Right-Fundus'], cropped_dir)
    
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"Execution time: {execution_time:.2f} seconds")

crop_train_images(df, "F:\BFPC/real_full\On-site Test Set\Images", "F:\BFPC/real_full\cropped")
